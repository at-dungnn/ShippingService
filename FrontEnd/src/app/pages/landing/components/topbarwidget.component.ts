  ...
  template: `
    ...
    <div class="items-center bg-surface-0 dark:bg-surface-900 grow justify-between hidden lg:flex absolute lg:static w-full left-0 top-full px-12 lg:px-0 z-20 rounded-border">
      <ul class="list-none p-0 m-0 flex lg:items-center select-none flex-col lg:flex-row cursor-pointer gap-8">
        ...
        <li>
          <a (click)="router.navigate(['/van-dong-vien'])" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
      ...
      <div class="flex border-t lg:border-t-0 border-surface py-4 lg:py-0 mt-4 lg:mt-0 gap-2">
        <button pButton pRipple label="Đăng nhập" routerLink="/auth/login" [rounded]="true" [text]="true"></button>
        <button pButton pRipple label="Đăng ký vận động viên" routerLink="/auth/login" [text]="true"></button>
        <button pButton pRipple label="Đăng ký giải đấu" routerLink="/auth/login" [text]="true"></button>
      </div>
    </div>
  `
})
